...
- (void)clickOkBtn{
    if (self.delegate && [self.delegate respondsToSelector:@selector(TKImageViewFinish:)]) {
        [self.delegate TKImageViewFinish:self.currentCroppedImage];
    }
}
- (void)clickCancelBtn{
    if (self.delegate && [self.delegate respondsToSelector:@selector(TKImageViewCancel)]) {
        [self.delegate TKImageViewCancel];
    }
}
...
- (UIImage *)currentCroppedImage {
    ...
    CGRect finalRect = CGRectMake((MINX(_cropAreaView) + _cropAreaBorderLineWidth) / scaleFactor, (MINY(_cropAreaView) + _cropAreaBorderLineWidth) / scaleFactor, (WIDTH(_cropAreaView) - 2 * _cropAreaBorderLineWidth) / scaleFactor, (HEIGHT(_cropAreaView) - 2 * _cropAreaBorderLineWidth) / scaleFactor);
    return [_toCropImage imageAtRect: finalRect];
}
