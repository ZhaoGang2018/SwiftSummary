 ...
 *  注意: 使用前请在'-Bridging-Header.h' 桥接文件中导入 #import<CommonCrypto/CommonCrypto.h>
 因为要用到MD5加密的东西
 */
let XHDataCacheKeyPath = "XHDataCacheKeyPath"

extension XHDataCache {
    /**
     ...
     - returns: 是否写入成功
     */
    public class func saveJsonResponseToCacheFile(_ jsonResponse: AnyObject, URL: String, path: String, subPath: String = "") -> Bool {
        
        let data = jsonToData(jsonResponse)
        let atPath =  cacheFilePathWithURL(URL, path: path, subPath: subPath)
     ...
     */
    public class func save_asyncJsonResponseToCacheFile(_ jsonResponse: AnyObject, URL: String, path: String , subPath: String = "", completed:@escaping (Bool) -> ()) {
        
        DispatchQueue.global().async{
            let result = saveJsonResponseToCacheFile(jsonResponse, URL: URL, path: path, subPath: subPath)
    ...
    public class func cacheJsonWithURL(_ URL: String,subPath:String = "") -> AnyObject? {
        if let keyPath = UserDefaults.standard.string(forKey: XHDataCacheKeyPath) {
    ...
    public class func cachePath() -> String {
        if let keyPath = UserDefaults.standard.string(forKey: XHDataCacheKeyPath) {
            let pathOfLibrary = NSSearchPathForDirectoriesInDomains(.cachesDirectory, .userDomainMask, true)[0] as NSString
...
class XHDataCache {
    ...
    fileprivate class func cacheFilePathWithURL(_ URL: String,
        ...
        if subPath.count == 0 {
            //保存最新的一级目录
            UserDefaults.standard.set(path, forKey: XHDataCacheKeyPath)
        ...
        //check路径
        let cacheFileNameString: String = "URL:\(URL) AppVersion:\(SpeedyApp.version)"
