//
//  XHRealmDatabaseManager+DataMigration.swift
//  XCamera
//
        ...
        migration.enumerateObjects(ofType: XHUploadRealmImageInfoModel.className()) { oldObject, newObject in
            ...
            if let oldModel = oldObject {
                ...
                if propertyNames.contains("fileData") && propertyNames.contains("user_id") && propertyNames.contains("fileName") {
                    ...
                    // 去掉fileData
                    if let imageData = oldObject?["fileData"] as? Data, let userId = oldObject?["user_id"] as? String, let fileName = oldObject?["fileName"] as? String {
                        let filePath = XHImageCacheManager.getPhotoCacheFilePath(userId: userId, fileName: fileName)
            ...
            if let oldModel = oldObject {
                ...
                if propertyNames.contains("picData") {
                    if let imageData = oldObject?["picData"] as? Data {
                        ...
                        _ = XHImageCacheManager.saveLogo(imageData: imageData, fileName: fileName)
